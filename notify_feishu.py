...
def _snapshot_summary() -> dict[str, object]:
    ...
    if not adjustments.empty:
        if "store" not in adjustments.columns:
            adjustments["store"] = ""
        if "store_level_ad_cost" not in adjustments.columns:
            adjustments["store_level_ad_cost"] = 0
        adjustments["store"] = adjustments["store"].fillna("").astype(str).str.strip()
        adjustments["store_level_ad_cost"] = (
            pd.to_numeric(adjustments["store_level_ad_cost"], errors="coerce")
            .fillna(0)
        )
        adjustments = (
            adjustments[adjustments["store"].ne("")]
            .groupby("store", as_index=False)
            .agg(store_level_ad_cost=("store_level_ad_cost", "sum"))
        )
        if not adjustments.empty:
            ...
            store_summary["store_level_ad_cost"] = (
                pd.to_numeric(store_summary["store_level_ad_cost"], errors="coerce")
                .fillna(0)
            )
            store_summary["ad_cost"] = (
                store_summary["ad_cost"]
                +
                store_summary["store_level_ad_cost"]
            )
            store_summary["profit"] = (
                store_summary["profit"]
                -
                store_summary["store_level_ad_cost"]
            )
