...
def _diagnosis_advice(row: pd.Series) -> list[str]:
    ...
    margin_rate = (pay_amount - cost_total) / pay_amount if pay_amount > 0 else 0
    ad_only_breakeven_roi = 1 / margin_rate if margin_rate > 0 else None
    ...
    if ad_cost > 0 and promotion_roi_value is not None:
        if ad_only_breakeven_roi and promotion_roi_value < ad_only_breakeven_roi:
            advice.append(
                f"仅看投放，当前ROI {_format_roi(promotion_roi_value)} 低于投放保本ROI {_format_roi(ad_only_breakeven_roi)}，建议降价出价或缩预算。"
            )
        elif ad_only_breakeven_roi:
            advice.append(
                f"仅看投放，当前ROI {_format_roi(promotion_roi_value)} 高于投放保本ROI {_format_roi(ad_only_breakeven_roi)}，可以小步加预算。"
            )
    elif pay_amount > 0:
    ...
    if overall_breakeven_roi:
        advice.append(f"整体保本ROI参考 {_format_roi(overall_breakeven_roi)}；投放单独保本ROI参考 {_format_roi(ad_only_breakeven_roi) if ad_only_breakeven_roi else '-'}。")
    return advice
...
def render_product_profit_consultant(realtime_rows: pd.DataFrame) -> None:
    ...
    rows = realtime_rows[realtime_rows["product_id"].astype(str).str.contains(query, case=False, na=False)].copy()
    if rows.empty:
    ...
    with advice_col:
        st.markdown("**调整建议**")
        for item in _diagnosis_advice(row):
            st.markdown(f"- {item}")
        st.markdown("**关键口径**")
        st.caption(
