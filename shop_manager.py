...
def make_shop_label_url(shop, start_url):
    name = str(shop.get("name", "")).strip() or "未命名店铺"
    port = str(shop.get("port", "")).strip()
    html = f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>{name} | 端口 {port}</title>
  <style>
    body {{
      margin: 0;
      min-height: 100vh;
    ...
    h1 {{ margin: 0 0 24px; font-size: 72px; letter-spacing: 0; }}
    .port {{
    ...
    .url {{ color: #cbd5e1; font-size: 22px; word-break: break-all; }}
  </style>
</head>
<body>
  <main>
...
def launch_shop_chrome(
    ...
    if platform == "douyin":
        default_start_url = "https://fxg.jinritemai.com/login"
    elif name == "国货严选":
        default_start_url = str(shop.get("sycm_url", "")).strip() or make_sycm_url()
    else:
        default_start_url = "https://loginmyseller.taobao.com/"

    start_url = (
        str(shop.get("start_url", "")).strip()
        or
        default_start_url
    )

    label_url = make_shop_label_url(
        shop,
        start_url
    )
